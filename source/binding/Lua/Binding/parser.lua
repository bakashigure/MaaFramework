...
local function parse_retrun_type(file_path, types)
    ...
    for _,v in pairs(basic_int_types) do
        types[v] = v
    end
...
local function ast2cpp_tail(funcs)
    local content = ""
    for _,v in ipairs(funcs) do
        content = content..string.format('        {"%s", %s},\n', v,v)
    end
    return string.format(cpp_tail, content)
end
...
local function run()
    ...
    for _,api in pairs(api_list) do
        ...
        cpp_content = cpp_content .. ast2cpp(func_ast, real_types, funcs)
        -- print(content)

    end

    cpp_content = cpp_content..ast2cpp_tail(funcs)
...
-- lua source/binding/Lua/Binding/parser.lua include/MaaFramework